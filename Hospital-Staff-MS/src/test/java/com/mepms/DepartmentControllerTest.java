...
import com.mepms.controller.DepartmentController;
...
public class DepartmentControllerTest {
    ...
    @Test
    public void getEquipmentsByDepartment_ShouldReturnEquipmentList() throws Exception {
        Equipment equipment1 = new Equipment();
        equipment1.setId("equip1");
        equipment1.setName("Equipment 1");
        
        Equipment equipment2 = new Equipment();
        equipment2.setId("equip2");
        equipment2.setName("Equipment 2");
        
        List<Equipment> equipments = Arrays.asList(equipment1, equipment2);
        
        Mockito.when(departmentService.getEquipmentsByDepartmentName("IT"))
                .thenReturn(equipments);

        mockMvc.perform(get("/api/departments/equipment/department/IT"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(2)))
                .andExpect(jsonPath("$[0].name", is("Equipment 1")))
                .andExpect(jsonPath("$[1].name", is("Equipment 2")));
    }
}